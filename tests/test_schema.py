...
def mock_response():
    ...
    mock_resp.json.return_value = {
        "current": "v1.0.0",
        "supported": ["v1.0.0", "v0.9.0"],  # Adding v0.9.0 for migration tests
        "deprecated": [],
...
class TestSchemaRegistry:
    ...
    def test_load_versions_info(self, mock_get, mock_response, temp_schema_dir):
        ...
        assert versions["current"] == "v1.0.0"
    ...
    def test_get_current_version(self, mock_response):
        ...
        with mock.patch.object(SchemaRegistry, 'load_versions_info', return_value=mock_response.json()):
            registry = SchemaRegistry()
            assert registry.get_current_version() == "v1.0.0"

    def test_get_supported_versions(self, mock_response):
        """Test getting supported versions."""
        with mock.patch.object(SchemaRegistry, 'load_versions_info', return_value=mock_response.json()):
            registry = SchemaRegistry()
            assert registry.get_supported_versions() == ["v1.0.0", "v0.9.0"]
    ...
    def test_load_schema(self, mock_get, mock_schema_response, temp_schema_dir):
        ...
        with mock.patch.object(SchemaRegistry, 'is_version_supported', return_value=True):
            ...
            # Test caching behavior
            _ = registry.load_schema(version="v1.0.0", schema_name="mediaplan.schema.json")  # Should use cached data
            assert mock_get.call_count == 1  # Should not call again
...
class TestSchemaMigrator:
    ...
    def test_register_migration(self):
        ...
        # Register it
        migrator.register_migration("v0.9.0", "v1.0.0", migration_func)

        # Check if it was registered
        assert migrator.can_migrate("v0.9.0", "v1.0.0")
        assert not migrator.can_migrate("v1.0.0", "v1.1.0")
    ...
    def test_find_migration_path_direct(self):
        ...
        migrator.register_migration("v0.9.0", "v1.0.0", lambda x: x)

        # Find path
        path = migrator.find_migration_path("v0.9.0", "v1.0.0")
        assert path == ["v1.0.0"]
    ...
    def test_find_migration_path_indirect(self):
        ...
        migrator.register_migration("v0.9.5", "v1.0.0", lambda x: x)

        # Find path
        path = migrator.find_migration_path("v0.9.0", "v1.0.0")
        assert path == ["v0.9.5", "v1.0.0"]
    ...
    def test_migrate_success(self):
        ...
        result = migrator.migrate(media_plan, "v0.9.0", "v1.0.0")
